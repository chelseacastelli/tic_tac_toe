...
def check_winner():
    ...
    for x in markers:
        # Check columns
        if sum(x) == 3:
            # Check if winner is player 1
            winner = 1
            game_over = True
        if sum(x) == -3:
            # Check if winner is player 2
            winner = 2
            game_over = True
...
while run:
    ...
    if game_over == True:
        draw_winner(winner)
        # Check for mouse click to see if user clicked on Play Again
        if event.type == pygame.MOUSEBUTTONDOWN and clicked == False:
            clicked = True
        if event.type == pygame.MOUSEBUTTONUP and clicked == True:
            clicked = False
            ...
            # If on click, mouse is in bounds of Play Again rectangle..
